...
impl Grid {
    ...
    pub fn scroll_up(&mut self, count: u16) {
        for _ in 0..(count.min(self.size.rows - self.scroll_top)) {
            ...
            // MINDPLAYER PATCH: upstream is `!self.scroll_region_active()`, which
            // discards lines scrolled out of ANY scroll region. TUIs like codex /
            // claude render their transcript inside a region anchored at the top of
            // the screen (with the input box pinned to the bottom rows), so their
            // history was lost and could never be scrolled back. Feed the scrollback
            // whenever the region's TOP is the top of the screen (scroll_top == 0) —
    ...
    // Unused after the MINDPLAYER PATCH in scroll_up (kept for upstream parity).
    #[allow(dead_code)]
